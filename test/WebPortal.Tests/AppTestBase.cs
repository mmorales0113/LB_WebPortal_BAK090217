...
using WebPortal.Tests.TestDatas;
...
namespace WebPortal.Tests
{
    ...
    /// Provides methods to easily work with <see cref="WebPortalDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<WebPortalTestModule>
    {
        ...
        {
            void NormalizeDbContext(WebPortalDbContext context)
            {
        ...
        protected void UsingDbContext(Action<WebPortalDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<WebPortalDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<WebPortalDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<WebPortalDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<WebPortalDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<WebPortalDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<WebPortalDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<WebPortalDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<WebPortalDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<WebPortalDbContext, Task<T>> func)
        {
            T result;

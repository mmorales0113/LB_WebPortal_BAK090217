...
namespace WebPortal.Authorization.Users
{
    ...
    /// </summary>
    public class UserEmailer : WebPortalServiceBase, IUserEmailer, ITransientDependency
    {
        ...
        public virtual async Task SendEmailActivationLinkAsync(User user, string link, string plainPassword = null)
        {
            ...
            if(user.IsActive == false || user.ServiceType == "Dental")
            {
                mailMessage.AppendLine("<b>" + L("YourAccountIsWaitingToBeActivatedByAdmin") + "<br />");
            }

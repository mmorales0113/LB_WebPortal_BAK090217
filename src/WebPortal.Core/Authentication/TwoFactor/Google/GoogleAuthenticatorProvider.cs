...
namespace WebPortal.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : WebPortalServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("WebPortal", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
